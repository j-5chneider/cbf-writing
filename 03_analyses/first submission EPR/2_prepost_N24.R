library(metafor)
data <- read_sav("C://Users//Salome Wagner//OneDrive - UT Cloud//Metaanalyse//cbf-writing//02_data//03_SPSS files//Metaanalysis_prepost_N23.sav")

####loop
data_pp <- data.frame(
  id = c(1,1,1,2,3,3,4,4,5,5,6,6,6,7,8,8,9,10,11,11,11,12,12,13),
  m_pre = data$M_pre,
  m_post = data$M_post,
  sd_pre = data$SD_pre,
...
for (ri_t in seq(from = .50, to=.75, by=.01)) {
  ...
  sensitivity <- sensitivity %>%
    add_row(ri_t = ri_t, 
            ...
            vi.f23 = rma_overall_clustered$vi.f[23],
            vi.f24 = rma_overall_clustered$vi.f[24],
...
rma_overall_clustered$yi.f[1:24] <- as.numeric(sensitivity_forest[1, 1:24])
rma_overall_clustered$vi.f       <- as.numeric(sensitivity_forest[1, 25:48])
rma_overall_clustered$beta[,1]   <- as.numeric(sensitivity_forest[1, 49])
rma_overall_clustered$vb[1,1]    <- as.numeric(sensitivity_forest[1, 50])

rma_overall_clustered
##result: g = .5295, p = .0129, CI 95% [0.1106, 0.9319] sign., medium effect
#Heterogeneity: Q(df = 23) = 588.4495, p < .0001

summary(sensitivity) #read mean ES from beta: .5775 (ranging from .57 to .58), p = .007 (ranging from .006 to .008), mean cor: r = .625 (ranging from .5 to .75)
...
#no missing studies on the left side, change of effect, now the effect of feedback on writing is sign. g = .6853, se = .1562, p = .0001, CI 95% [0.3792, 0.9914]
#Heterogeneity: Q(df = 21) = 281.2100, p-val < .0001

##Egger's test
regtest(rma_funnel) #asymmetry z = 4.6283, p < .0001, b = -0.1198 (CI: -0.5189, 0.2792)
...
#I^2 = 98.41% of the detected variation could be related to  true variation among studies
...
data_pp_es$numeric<- as.factor(data$FB_order)


rep<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = rep)
rep #sign. g = -.0759, p = .0112
rep_nr<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = rep_nr)
rep_nr
order<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = order)
order #sign. g = -.5169, p < .001
low_order<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = order_low)
low_order #sign. g = .4573, p = .0025
hi_order<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = order_hi)
hi_order #sign. g = -.0431, p = .7013
hilow_order<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = order_hilow)
hilow_order #not sign. g = -.6143, p = .0002
spec<-rma.mv(yi, vi, data=data_pp_es, random = ~ 1 | id, mods = spec)
spec #not sign. g = .6252, p = .0001
...
vorwissen #not sign. g = .0297, p = .2202
