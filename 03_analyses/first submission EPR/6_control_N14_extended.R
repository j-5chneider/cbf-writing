dataN14 <- read_sav("C://Users//Salome Wagner//OneDrive - UT Cloud//Metaanalyse//cbf-writing//02_data//03_SPSS files//Metaanalysis_control_N14.sav")
#### LOOP BEGIN ################################################################
datT <- data.frame(
  sampleNr = 1:14,
...
for (ri_t in seq(from = .50, to=.75, by=.01)) {
  ...
  dat$id <- c(1,1,1,2,3,3,4,4,5,6,7,8,8,8)
  dat$setting <- as.factor(dataN14$setting)
...
for (ri_t in seq(from = .50, to=.75, by=.01)) {
  ...
  dat$id <- c(1,1,1,2,3,3,4,4,5,6,7,8,8,8)
  dat$education <- as.factor(dataN14$education)
