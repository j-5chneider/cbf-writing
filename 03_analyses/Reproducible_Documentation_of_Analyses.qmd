...
for (mod in moderators) {
  result <- fn_meta_pre_post(moderator = mod, cor = r_prepost, data_pp_es = data_pp)
...
# V <- vcalc(vi = vi, cluster = author, subgroup = group, data = dat)
...
for (mod in moderators) {
  result <- fn_meta_experimental(moderator = mod, cor_exp = r_exp, 
                                 cor_con = r_con, data_c = data_c) %>%
    tidy()
  result$moderator <- mod
  results_moderation <- bind_rows(results_moderation, result)
}
...
experimental_final <- results_moderation %>%
  mutate(term = str_remove(term, "^[^)]*\\)")) %>%
  mutate(sig = case_when(
    p.value < .001 ~ "***",
    p.value < .01 ~ "**",
...
for (mod in moderators) {
  result <- fn_meta_experimental(moderator = mod, cor_exp = r_exp, cor_con = r_con, data_c = data_c)
